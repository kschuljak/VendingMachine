...
- A hidden menu option displays a current sales report.  An up-to-date sales report is written to a .csv file whenever the hidden menu option is called or the user selects the option to exit the program.  When the program is next run, the information from the .csv file is read and stored in memory until a new sales report is printed to overwrite the old file.
